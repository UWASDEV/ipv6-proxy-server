...
function get_subnet_mask(){
  if [ -z $subnet_mask ]; then
    ...
    # Skip the provider's SLAAC/RA address (deprecated/temporary/mngtmpaddr) and
    # any tentative one: those are not stable across reboots and are usually not
    # routed to this host, so basing the proxy pool on them breaks the proxy on
    # some VPSes. Prefer a stable, statically-configured global address.
    ipv6=$(ip -6 addr show scope global 2>/dev/null | grep inet6 | grep -viE 'deprecated|temporary|tentative|dadfailed|mngtmpaddr' | awk '{print $2}' | grep -v '/128' | cut -d '/' -f1 | grep -m1 -viP '^fe80');
    # Fallback to the original parsing if the stricter filter excluded everything.
    if [ -z "$ipv6" ]; then
...
function create_network_persistence_service(){
  # Cloud VPSes (cloud-init + systemd-networkd / netplan, NetworkManager, ...)
  # periodically re-apply their own network config - on DHCPv6 lease renewal, on
  # `netplan apply`, on reboot - and FLUSH everything they don't own: the extra
  # pool IPv6 addresses, the local route and the per-interface sysctls. That
  # silently breaks the proxy until the next rotation. This background service
  # re-applies them every few seconds, so the proxy heals within seconds of any
  # such flush. It is network-manager agnostic (works on netplan/networkd,
  # NetworkManager and ifupdown alike), because it just re-asserts the state.
  local ensure_script_path="$proxy_dir/ensure-ipv6.sh";
...
#!$bash_location
# Auto-generated by ipv6-proxy-server.sh.
# Re-applies the IPv6 networking the proxy needs so it survives network-manager
# reconfigurations / reboots. Run with --loop by the systemd service.
interface_name="$interface_name"
subnet="$subnet"
subnet_mask="$subnet_mask"
random_ipv6_list_file="$random_ipv6_list_file"
ndppd_routing_file="$ndppd_routing_file"
EOF

  # Body (literal): the logic is identical on every server.
  cat >> $ensure_script_path <<'EOF'

reassert_once() {
  sysctl -qw "net.ipv6.conf.${interface_name}.proxy_ndp=1"  2>/dev/null
  sysctl -qw "net.ipv6.conf.all.proxy_ndp=1"                2>/dev/null
  sysctl -qw "net.ipv6.conf.${interface_name}.forwarding=1" 2>/dev/null
...
if [ "$1" = "--loop" ]; then
  while true; do reassert_once; sleep 3; done
else
  reassert_once
fi
EOF

  chmod +x $ensure_script_path;

  cat > $service_path <<EOF
[Unit]
Description=Keep IPv6 proxy-server pool addresses and sysctls applied (self-heal against network-manager flushes)
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
ExecStart=$bash_location $ensure_script_path --loop
Restart=always
RestartSec=3
...
EOF
  ...
  systemctl enable --now $service_name.service &>> $script_log_file;

  if systemctl is-active --quiet $service_name.service; then
    echo "IPv6 self-heal service installed and running ($service_name)";
  else
    log_err "Warning: could not start IPv6 self-heal service ($service_name).";
  fi;
}

function remove_network_persistence_service(){
  local service_name="ipv6-proxy-server-net";
  systemctl disable --now $service_name.service &>> $script_log_file;
  delete_file_if_exists "/etc/systemd/system/$service_name.service";
  systemctl daemon-reload &>> $script_log_file;
}
...
if $uninstall; then
  ...
  remove_from_cron;
  remove_network_persistence_service;
  kill_3proxy;
...
run_proxy_server;
create_network_persistence_service;
